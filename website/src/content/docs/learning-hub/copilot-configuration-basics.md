...
### Editor Mode and Interface Settings (v1.0.85+)

**Vim mode** is now available to everyone. Enable it with `/vim` or by setting `editorMode` to `vim`, which turns the composer into a modal editor (normal/insert modes) with the current mode shown while you type.

The new `/config` command opens a sidebar configuration screen inside the CLI, giving you a persistent view of settings alongside your session instead of a full-screen dialog like `/settings`.

`/settings` also gained an option to opt in to **context management tools** for agents and subagents, and a `transcriptView` setting of `"concise"` groups tool activity into expandable work summaries instead of showing every tool call inline — useful for long sessions with heavy tool use.

`/sandbox` now supports network host allow/deny rules that layer on top of your configured upstream proxy instead of replacing it, so you can fine-tune which hosts a sandboxed session can reach without losing your existing proxy setup.
