...
copilot skill add ./my-skill/ --project # add a skill scoped to this project
copilot skill remove my-skill           # remove an installed skill by name
copilot skill enable my-skill           # enable a previously disabled skill
copilot skill disable my-skill          # disable a skill without uninstalling it
```
...
> **Breaking change (v1.0.85+)**: `copilot plugins install --skill [--scope project]` has been replaced by `copilot skill add [--project]` — the `--scope` spelling is gone. Use `copilot skill enable`/`disable` in place of the old `copilot plugins enable/disable --skill` flag.
