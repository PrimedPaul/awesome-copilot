...
flowchart LR
  subgraph fork [PrimedPaul/awesome-copilot]
    ...
    pr --> lifecycle[fork-plan-lifecycle-check<br/>deterministic approval gate]
    pr --> vally[skill-check vally lint<br/>upstream workflow]
    pr --> rev[fork-agent-reviewer<br/>gh-aw: integrity/domain review + version check]
...
| `fork-agent-reviewer.md` → `.lock.yml` | gh-aw | `pull_request` into `main`, path-scoped to the agent, plugin, and `skills/*oracle-to-postgres*` | `version_check` job fails if `plugin.json` version equals upstream. AI agent verifies integrity, Oracle→PostgreSQL domain correctness, safety boundaries, rule consistency, and promotion readiness, then submits one review (`COMMENT` or `REQUEST_CHANGES`; can never `APPROVE`). |
| `fork-plan-lifecycle-check.yml` | plain YAML | `pull_request` into `main`, path-scoped to plans, the agent, plugin, and `skills/*oracle-to-postgres*` | Deterministic (`eng/validate-fork-plan-lifecycle.mjs`, no AI agent) check that a plan's `status: approved` claim is backed by machine-readable lifecycle evidence — requirements answered, review completed/waived, and a matching `/approve-plan` comment from the repo owner with a content hash — and that `Fixes #N` is only armed once that evidence checks out and GitHub's own `closingIssuesReferences` agrees. See `.github/fork-only/plans/README.md`. |
...
Plan files under `.github/fork-only/plans/` were originally **advisory artefacts, not state**: nothing read them automatically and nothing branched on their contents, so a stale plan was a stale note. That changed for the `approved` claim specifically: `fork-plan-lifecycle-check.yml` now reads a plan's `lifecycle` frontmatter and fails the PR if `status: approved` isn't backed by evidence (requirements answered, review completed/waived, a matching `/approve-plan` comment, and a content hash that still matches). The plan's *prose* — the grilling, the plan itself, the acceptance criteria — remains advisory; only the approval claim is now enforced. See `.github/fork-only/plans/README.md` for the full contract.
